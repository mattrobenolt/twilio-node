...
var RequestValidator = function(token)
{
    this.token = token;
};
RequestValidator.prototype = {
    compute_signature: function(uri, params)
    {
        var s = uri;
        if(params)
        {
        ...
        var hash = crypto.createHmac('sha1', this.token);
        hash.update(s);
        return hash.digest('base64');
    },

    validate: function(uri, params, signature)
    {
        return this.compute_signature(uri, params) == signature;
    }
};
...
exports.TwilioCapability = TwilioCapability;